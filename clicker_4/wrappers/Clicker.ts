...
export function clickerConfigToCell(config: ClickerConfig): Cell {
    return beginCell().storeRef(config.trackerCode).storeMaybeRef(config.dic).endCell();
}
...
export class Clicker implements Contract {
    ...
    async getDic(provider: ContractProvider): Promise<{ [key: number]: number }> {
        const result = (await provider.get('get_dic', [])).stack;
        const accountsRaw = result.readCellOpt();
        const addressList: { [key: number]: number } = {};

        if (accountsRaw !== null) {
            const addrMap = parseDict(accountsRaw.beginParse(), 64, (slice) => {
                return slice.loadUint(32);
            });

            for (const [keyBigInt, value] of addrMap.entries()) {
                const key = Number(keyBigInt);
                addressList[key] = value;
            }
        }
        return addressList;
    }
